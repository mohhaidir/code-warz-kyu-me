...
function goodVsEvil(good, evil) {
    ...
    let human = (goodArmy[0] * 1) + (goodArmy[1] * 2) + (goodArmy[2] * 3) + (goodArmy[3] * 3) + (goodArmy[4] * 4) + (goodArmy[5] * 10)
    let nonHuman = (evilArmy[0] * 1) + (evilArmy[1] * 2) + (evilArmy[2] * 2) + (evilArmy[3] * 2) + (evilArmy[4] * 3) + (evilArmy[5] * 5) + (evilArmy[6] * 10)
    if (human > nonHuman) {
        return 'Battle Result: Good triumphs over Evil'
    } else if (nonHuman > human) {
        return 'Battle Result: Evil eradicates all trace of Good'
    } else if (human == nonHuman) {
